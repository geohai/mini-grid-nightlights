...
## Summary Table
```{r}
df <- readRDS("data/dark_bg/nightlights_means_medians.rds")

# create a summary table 
df_sum <- df %>% pivot_wider(names_from = type, values_from = c(mean, median))
...
## Print Median Values for Ocean Background
```{r}
# read in global means
global_means <- readRDS("data/dark_bg/nightlights_means_medians.rds")

# keep just ocean type from global means
global_means <- global_means %>%
    filter(type == "ocean") %>%
    select(date, median, mean)

# print the entire df for median and mean values
print(global_means, n=121)
```