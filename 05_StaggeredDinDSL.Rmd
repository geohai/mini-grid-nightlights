...
```{r}
# read in data
df <- read_csv("data/sl/nightlight_values_v2.csv")
# View(df)
...
# check for duplicates in site_name
dups <- df %>%
    count(site_name) %>%
    filter(n > 117)
View(dups)
# manually check to see if the nightlight values match up >> they do >> duplicated

# drop the CBIL versions of the duplicated sites
df <- df %>%
    filter(!(site_name %in% dups$site_name & site_type == "CBIL"))
...
dups <- df %>%
    ...
    filter(n > 117)
View(dups)
# this dropped 6/7 of the duplicated sites
# there is one more that appears in the list of "Control" sites >> oops! Remove it too
df <- df %>%
    filter(!(site_name %in% dups$site_name & site_type == "Control"))
...
# all good now! Re-export the data
write_csv(df, "data/sl/nightlight_values_v2_clean.csv")
```
...
```{r}
# read in data
df <- read_csv("data/sl/nightlight_values_v2_clean.csv")
...
# export df to csv
write_csv(df, "data/sl/nightlight_values_v2_clean_no_outliers.csv")
```
...
# set the date commissioned for all Control sites to be one month before the earliest date
df$date_commissioned[df$site_type == "Control"] <- start_date - months(1)
...
#### ADD IN THE DARK AREA MEAN AS A COVARIATE?????

# find which rows have NA for group
...
# export df to csv
# View(df)
write_csv(df, "data/sl/nightlight_values_for_csdind.csv")
```

## Run the Analysis
```{r}
# read in the data
df <- read_csv("data/sl/nightlight_values_for_csdind.csv")
...
# export df to csv
write_csv(df, "data/sl/nightlight_values_for_csdind_years.csv")
```

## Rerun the Analysis
```{r}
# read in the data
df <- read_csv("data/sl/nightlight_values_for_csdind_years.csv")
