...
## Visualize Data Before Cleaning Outliers
```{r}
# read in data
africa_df_melt <- readRDS("./data/dark_africa/africa_df_melt.rds")
df <- africa_df_melt
...
# # filter out all observations with type = "built"
df <- df %>%
    filter(type != "built")
...
saveRDS(did_control, "./data/dark_africa/did_control.rds")
...
# export the results
saveRDS(did_control_agg, "./data/dark_africa/did_control_agg.rds")
...
## Rerun with just Certain Types as Controls
```{r}
# read in the data
africa_df_melt_clean_for_csdind <- readRDS("./data/dark_africa/africa_df_melt_clean_for_csdind.rds")
df <- africa_df_melt_clean_for_csdind

# keep only types minigrid and built
df <- df %>%
    filter(type == "minigrid" | type == "jungle")
...
# export the model results
saveRDS(did_control, "./data/dark_africa/did_control_minigrid_jungle.rds")
...
# export the results
saveRDS(did_control_agg, "./data/dark_africa/did_control_agg_minigrid_jungle.rds")
...
# plot group-time ATTs
did_control_agg_plt <- ggdid(did_control_agg) +
    labs(title = "Group-Time ATTs for Minigrid vs Rainforest", x = "Time Period", y = "ATT") +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
...
## Rerun Using Not Yet Treated as Control
```{r}
# read in the data
africa_df_melt_clean_for_csdind <- readRDS("./data/dark_africa/africa_df_melt_clean_for_csdind.rds")
df <- africa_df_melt_clean_for_csdind
...
# export the model results
saveRDS(did_nyt, "./data/dark_africa/did_nyt_minigrid.rds")
...
# export the results
saveRDS(did_nyt_agg, "./data/dark_africa/did_nyt_agg_minigrid.rds")

# view the results
summary(did_nyt_agg)

# plot group-time ATTs
did_nyt_agg_plt <- ggdid(did_nyt_agg) +
    labs(title = "Group-Time ATTs for Mini-Grid Sites vs Not Yet Treated Sites", x = "Time Period", y = "ATT") +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))

print(did_nyt_agg_plt)
```
...
did_nyt_agg_plt_minigrid <- ggdid(did_nyt_agg_minigrid, xgap = 6) +
    labs(title = "Average Effect of Mini-grid Construction on Nighttime Brightness for 700+ Sites in Africa", x = "Months Since Commissioning", y = "Change in Brightness") +
    theme_bw() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
...
# export one
ggsave("figures/dark_africa_v2/did_nyt_agg_plt_minigrid_hires.png", did_nyt_agg_plt_minigrid, width = 12, height = 6, units = "in", dpi = 900)
```
