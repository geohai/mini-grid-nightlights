...
## Read in Dark Area Data, Clean, Re-export
```{r}
# read in data
africa_rural_df <- read_csv("data/dark_africa/nighttime_rural_melt_df.csv")
africa_dark_df <- read_csv("data/dark_africa/nighttime_dark_melt_df.csv")

# glimpse the dfs
glimpse(africa_rural_df)
glimpse(africa_dark_df)

# merge the dfs
dark_df <- bind_rows(africa_rural_df, africa_dark_df)
glimpse(dark_df)
rm(africa_dark_df, africa_rural_df)
...
dark_df <- dark_df %>%
    select(-Map)
# set type to factor
dark_df$type <- as.factor(dark_df$type)
...
# update the id column to include the type
dark_df <- dark_df %>%
    mutate(id = as.factor(paste0(type, "_", id)))

# the coordinates are stored in the column "center" as <chr> "[40.985976795541156, 11.365131764777153]", convert this to sf object
dark_sf <- dark_df %>%
    mutate(center = str_remove_all(center, "\\[|\\]")) %>%
...
glimpse(dark_sf)
# export df to csv
write_csv(dark_df, "./data/dark_africa/dark_df.csv")
# export sf to rds
saveRDS(dark_sf, "./data/dark_africa/dark_sf.rds")
```

## Merge with Mini-Grid Data
```{r}
# read in data
africa_minigrid_df <- read_csv("data/cluber/nighttime_cluber_df_melt.csv")
africa_minigrid_landcover_df <- read_csv("data/cluber/landcover_cluber_gdf.csv")
# glimpse the df
glimpse(africa_minigrid_df)

# create an id column with an integer for each unique site_name
africa_minigrid_df <- africa_minigrid_df %>%
    mutate(id = as.integer(as.factor(site_name)))

# convert into sf, use column "geometry" as is
africa_minigrid_sf <- st_as_sf(africa_minigrid_df, wkt = "geometry")
...
# convert image_date to date
africa_minigrid_sf$image_date <- as.Date(africa_minigrid_sf$image_date)

# drop status, developer, customers, and country column
africa_minigrid_sf <- africa_minigrid_sf %>%
    select(-status, -developer, -customers, -country, -source, -site_name)

# add in type column with value "minigrid"
africa_minigrid_sf <- africa_minigrid_sf %>%
    mutate(type = "minigrid")
# add in a landcover column with value NA
africa_minigrid_sf <- africa_minigrid_sf %>%
    mutate(landcover = NA)
st_crs(africa_minigrid_sf) <- 4326
# add in type to id column
africa_minigrid_sf <- africa_minigrid_sf %>%
    mutate(id = as.factor(paste0(type, "_", id)))
# drop that first random column
africa_minigrid_sf <- africa_minigrid_sf %>%
    select(-1)
...
# read in dark_sf
dark_sf <- readRDS("./data/dark_africa/dark_sf.rds")
# add in a date_commissioned column with value NA
dark_sf <- dark_sf %>%
    mutate(date_commissioned = NA)
glimpse(dark_sf)
# set crs to 4326
st_crs(dark_sf) <- 4326

# combine the two dfs
africa_sf <- bind_rows(dark_sf, africa_minigrid_sf)
