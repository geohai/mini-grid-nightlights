...
# create a unique integer id from the geometry column
sl_pop_melt_sf$id <- as.integer(as.factor(as.character(sl_pop_melt_sf$geometry)))
...
# visualize as a time series before cleaning
sl_pop_melt_sf %>%
    # filter(type != "built") %>%
    ggplot(aes(x = year, y = population, color = site_name)) +
    geom_line() +
    labs(title = "Population Values Over Time in Sierra Leone Before Cleaning", x = "Year", y = "Population Estimate (1km Buffer)") +
    theme_bw() + 
    # hide legend
...
ggsave("figures/pop_sl/pop_by_year_by_site_type_serif_all.png", width = 12, height = 8, units = "in", dpi = 300)

# get summary statistics by site_type sd, mean, min, max, median
...
summmary_stats <- summmary_stats %>%
    mutate_if(is.numeric, ~round(., 0))

print(summmary_stats)
```
...
glimpse(df)
# keep just mini-grids and built
df <- df %>%
    filter(type %in% c("mini-grid", "built"))
...
did_agg_plt <- ggdid(did_agg) +
    labs(title = "ATTs for Population for Mini-Grids versus Built-Up Areas in Sierra Leone", x = "Years Since Commissioning", y = "Change in Population") +
    theme_bw() +
...
# export to png
ggsave("figures/pop_sl/did_mg_built_serif.png", did_agg_plt, width = 10, height = 8, units = "in", dpi = 300)
```
...
Overall summary of ATT's based on event-study/dynamic aggregation:  
 ...
 9.0895        21.401   -32.8557     51.0346 

### Not Yet Treated Group
Overall summary of ATT's based on event-study/dynamic aggregation:  
      ATT    Std. Error     [ 95%  Conf. Int.]  
 6.0153       21.7654   -36.6442     48.6747 

 ## Mini-Grids vs. Rural Areas
      ATT    Std. Error     [ 95%  Conf. Int.]  
 57.4288       18.6379    20.8991     93.9584 *

 ## Mini-Grids vs. Urban Areas
        ATT    Std. Error     [ 95%  Conf. Int.]  
 -390.6164      107.7614   -601.825   -179.4079 *