...
country_iso3=ZWE
year=2020
aws s3 cp s3://globalnightlight/HREAv1.1_COGs/${country_iso3}/${country_iso3}/set_lightscore/${country_iso3}_set_lightscore_${year}.tif data/hrea/ --no-sign-request
aws s3 cp s3://globalnightlight/HREAv1.1_COGs/${country_iso3}/${country_iso3}/set_lightscore/${country_iso3}_set_lightscore_${year}.json data/hrea/ --no-sign-request
```
...
# create a 1000m buffer around the mini-grid points
...
```{r}
high <- mgs_sl_buff_1000[16,]
high$site_name # "Sahn"

# buffer high by another 200 meters
...
sat_rast_high <- rast(sat_map_high)
plot(sat_rast_high)
...
```{bash}
year=2020
tail -n +2 data/hrea/countries.csv | cut -d "," -f 2 | tr -d '"' | while read country_iso3
do
    echo "Downloading lightscore data for country: $country_iso3 in year $year"
    aws s3 cp s3://globalnightlight/HREAv1.1_COGs/${country_iso3}/${country_iso3}/set_lightscore/${country_iso3}_set_lightscore_${year}.tif data/hrea/ --no-sign-request
    aws s3 cp s3://globalnightlight/HREAv1.1_COGs/${country_iso3}/${country_iso3}/set_lightscore/${country_iso3}_set_lightscore_${year}.json data/hrea/ --no-sign-request
done
```
...
for (country_iso3 in countries_w_codes$ISO3166.1.Alpha.3) {
    ...
    mgs_country_buff <- buffer(mgs_country, 1000)
    # extract the values (max and mean)
    result_mean <- terra::extract(ls, mgs_country_buff, fun = mean, na.rm = TRUE)
    result_max <- terra::extract(ls, mgs_country_buff, fun = max, na.rm = TRUE)
    # join to the mini-grids
    mgs_country$lightscore_mean_1000 <- result_mean$layer
    mgs_country$lightscore_max_1000 <- result_max$layer
    # convert mgs_country SpatVector to a data frame
    # print(paste0("Converting to data frame..."))
    mgs_country_df <- as.data.frame(mgs_country)
    # print(paste0("Merging into resutls..."))
    mgs_results[[country_iso3]] <- mgs_country_df
}
mgs_results
# combine results into a single df
mgs_results_df <- do.call(rbind, mgs_results)
mgs_results_df$date_commissioned <- as.POSIXct(as.numeric(mgs_results_df$date_commissioned), origin="1970-01-01", tz="UTC")

# remove mini-grids that have invalid lightscore values
# starts with 473 mini-grids
mgs_results_df <- mgs_results_df[!is.na(mgs_results_df$lightscore_mean_1000), ]
mgs_results_df <- mgs_results_df[!is.na(mgs_results_df$lightscore_max_1000), ]
# 435 mini-grids

# export
write.csv(mgs_results_df, "data/hrea/mgs_results.csv", row.names = FALSE)
```

## Visualize the Resutls
```{r}
# read in the results
mgs_results_df <- read.csv("data/hrea/mgs_results.csv")
mgs_results_df$date_commissioned <- as.Date(mgs_results_df$date_commissioned)

# create a density plot of the lightscore
ggplot(mgs_results_df) +
  geom_histogram(aes(lightscore_mean_1000), binwidth = 0.1, fill = "darkgreen", col = "darkgreen", alpha = 0.5) +
  geom_histogram(aes(lightscore_max_1000), binwidth = 0.1, fill = "darkblue", col = "darkblue", alpha = 0.5) +
  theme_bw() +
  labs(
    title = "Histogram of Lightscore for 400+ Mini-Grids in SSA",
    x = "% Likelihood of Electrification ('Lightscore')",
    y = "Count"
  ) +
  # y ticks every 1, x tickets every 0.1
...
# Calculate the number of mini-grids with a lightscore < 0.5
n_unelectrified_per_mean_lightscore <- mgs_results_df %>%
    filter(lightscore_mean_1000 < 0.5) %>%
    nrow()

n_unelectrified_per_max_lightscore <- mgs_results_df %>%
    filter(lightscore_max_1000 < 0.5) %>%
    nrow()

n_minigrids <- nrow(mgs_results_df)

percent_unelectrified_per_mean_lightscore <- n_unelectrified_per_mean_lightscore / n_minigrids * 100
percent_unelectrified_per_max_lightscore <- n_unelectrified_per_max_lightscore / n_minigrids * 100

# print the results
print(paste0("Number of Mini-Grids with Lightscore < 0.5 (Mean): ", n_unelectrified_per_mean_lightscore))
print(paste0("Number of Mini-Grids with Lightscore < 0.5 (Max): ", n_unelectrified_per_max_lightscore))
print(paste0("Percent of Mini-Grids with Lightscore < 0.5 (Mean): ", percent_unelectrified_per_mean_lightscore))
print(paste0("Percent of Mini-Grids with Lightscore < 0.5 (Max): ", percent_unelectrified_per_max_lightscore))
```

## Create Summary Table of Results by Country
```{r}
# create a summary table of the results by country
mgs_results_summary <- mgs_results_df %>%
    group_by(country) %>%
    summarise(
        n_mini_grids = n(),
        n_electrified_mean = sum(lightscore_mean_1000 >= 0.5),
        n_electrified_max = sum(lightscore_max_1000 >= 0.5),
        percent_electrified_mean = n_electrified_mean / n_mini_grids * 100,
        percent_electrified_max = n_electrified_max / n_mini_grids * 100
    )
# export the summary table
write.csv(mgs_results_summary, "data/hrea/mgs_results_summary.csv", row.names = FALSE)

# print the overall summary not by country
mgs_results_summary_overall <- mgs_results_summary %>%
    summarise(
        n_mini_grids = sum(n_mini_grids),
        n_electrified_mean = sum(n_electrified_mean),
        n_electrified_max = sum(n_electrified_max),
        percent_electrified_mean = n_electrified_mean / n_mini_grids * 100,
        percent_electrified_max = n_electrified_max / n_mini_grids * 100
    )
print(mgs_results_summary_overall)
# export the overall summary
write.csv(mgs_results_summary_overall, "data/hrea/mgs_results_summary_overall.csv", row.names = FALSE)
