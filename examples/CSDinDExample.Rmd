...
```{r}
original_file_name <- "/Users/ilyonsg/Documents/research/thesis/CB_Lab_Mapping_Economic_Migration/data/dark_sl/all_df_for_csdind.csv"
clean_monthly_file_name <- "/Users/ilyonsg/Documents/research/thesis/CB_Lab_Mapping_Economic_Migration/data/dark_sl/all_df_for_csdind_nonames_monthly.csv"
clean_yearly_file_name <- "/Users/ilyonsg/Documents/research/thesis/CB_Lab_Mapping_Economic_Migration/data/dark_sl/all_df_for_csdind_nonames_yearly.csv"
```
...
# unique(df$type)
# # "rural"     "built"     "ocean"     "control"   "mini-grid"
...
# visualize the image_value over time with image_date by grouped by type
df %>%
...
# keep just two site types
# possible values: "rural", "built", "ocean", "control", "mini-grid"
df <- df %>%
...
did_control_agg_plt <- ggdid(did_control_agg, xgap = 6) +
    labs(
        title = "Average Treatment Effect (ATT) of Mini-Grid Construction on Nighttime Brightness for Sierra Leone Mini-Grid Sites", 
...
Overall summary of ATT's based on event-study/dynamic aggregation:  
  ...
  0.0186        0.0136    -0.0081      0.0453

### Not Yet Treated Group
Overall summary of ATT's based on event-study/dynamic aggregation:  
     ATT    Std. Error     [ 95%  Conf. Int.] 
   0.018        0.0142    -0.0098      0.0458
...
df <- df %>%
    group_by(id, type, year_image, year_commissioned) %>%
    summarize(
...
# https://bcallaway11.github.io/did/reference/att_gt.html
did_control <- att_gt(
    yname = "image_value_mean", # outcome variable
    tname = "time_period", # time variable
    idname = "id", # id variable
    gname = "group", # first treatment period variable
    data = df, # data
