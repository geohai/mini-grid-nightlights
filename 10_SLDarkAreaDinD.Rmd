...
# set date commissioned to 2013-12-01 for control sites
grid_sf$date_commissioned[grid_sf$site_type == "Control"] <- as.Date("2013-12-01")
...
df %>%
    ...
    geom_line() +
    labs(title = "Brightness Values Over Time in Sierra Leone Before Cleaning", x = "Image Date", y = "Image Value") +
    theme_bw() + 
    # hide legend
...
## Revisualize after Cleaning
```{r}
# read in the data
df <- readRDS("data/dark_sl/all_df_melt_clean.rds")
...
# scatter plot with geom smooth for just mgs and controls
df %>%
    filter(type == "mini-grid" | type == "control") %>%
    ggplot(aes(x = image_date, y = image_value, color = type)) +
    geom_point() + 
    geom_smooth() +
    labs(title = "Nightlight Image Values Over Time After Cleaning", x = "Image Date", y = "Image Value") +
    theme_bw() + 
    theme(text = element_text(size = 28, family = "serif")) +
...
values_by_type_plot <- df %>%
    ...
    ggplot(aes(x = image_date, y = image_value, color = type)) +
    geom_smooth() +
    labs(title = "Nighttime Brightness Values Over Time by Site Type", x = "Image Date", y = "Nighttime Brightness") +
    theme_bw()
values_by_type_plot
# export for prezi
ggsave("figures/dark_sl/values_by_type_plot_hi_res.png", values_by_type_plot, width = 12, height = 6, units = "in", dpi = 900)
# remake for paper
values_by_type_plot + theme(text = element_text(size = 28, family = "serif"))
...
# plot histogram of group variable grouped by si
# set y max to 120
df %>% 
    filter(type == "mini-grid" | type == "control") %>%
    filter(image_date == "2014-01-01") %>% # just one row per site   
    ggplot(aes(x = group)) +
    geom_histogram(binwidth = 1) +
    xlim(-1, 120) +
    # set vertical major gridlines to 10
    scale_x_continuous(breaks = seq(0, 120, by = 10)) +
    labs(title = "Histogram of Group Variable for Nightlight Analysis in Sierra Leone", x = "Group", y = "Count") +
    # size 20 font
    theme_bw() +
    theme(text = element_text(size = 28, family = "serif"))
...
write.csv(df, "data/dark_sl/all_df_for_csdind.csv", row.names = FALSE)
```
...
df <- df %>%
    filter(type %in% c("mini-grid", "control"))
...
did_control_agg_plt <- ggdid(did_control_agg, xgap = 6) +
    labs(title = "ATT of Mini-Grid Commissioning on Nighttime Brightness for Mini-Grid Sites vs. Not-Yet Treated Areas", x = "Months Since Commissioning", y = "Change in Brightness") +
    theme_bw() +
...
ggsave("figures/dark_sl/did_minigrid_vs_self_serif.png", did_control_agg_plt, width = 16, height = 8, units = "in", dpi = 300)

# export as a figure for thesis
# ggsave("figures/dark_sl/did_minigrid_vs_control_v3.png", did_control_agg_plt, width = 12, height = 6, units = "in", dpi = 600)
...
### Mini-Grid vs Control Sites
 ...
 0.0186         0.014    -0.0089      0.0461 

### Mini-Grids vs. Control + Not-Yet Treated Sites
   ATT    Std. Error     [ 95%  Conf. Int.] 
 0.018        0.0137    -0.0089      0.0448 

### Mini-Grids vs. Not-Yet Treated Sites
    ATT    Std. Error     [ 95%  Conf. Int.] 
 0.0166        0.0198    -0.0223      0.0554 

### Mini-Grid vs Rural Areas
    ATT    Std. Error     [ 95%  Conf. Int.] 
 0.0159        0.0143    -0.0122       0.044 

### Mini-Grid vs Ocean Areas
    ATT    Std. Error     [ 95%  Conf. Int.] 
 0.0091        0.0147    -0.0197       0.038 

### Mini-Grid vs Built Areas
     ATT    Std. Error     [ 95%  Conf. Int.]  
 -0.3882        0.0902     -0.565     -0.2113 *
...
dens_plot <- df %>%
    ...
    theme(text = element_text(size = 20, family = "serif"))
dens_plot
ggsave("figures/dark_sl/bright_values_by_type_density_serif.png", dens_plot, width = 16, height = 8, units = "in", dpi = 300)

# create a q-q plot of image values by type
qq_plot <- df %>%
    ggplot(aes(sample = image_value, color = type)) +
    stat_qq() +
    ...
    ylim(-0.5, 3) +
    xlim(-4,4) +
    labs(title = "Q-Q Plot of Image Values by Type", x = "Theoretical Quantiles", y = "Sample Quantiles") +
    theme_bw() +
    theme(legend.position = "bottom") +
    theme(text = element_text(size = 20, family = "serif"))
qq_plot
ggsave("figures/dark_sl/bright_values_by_type_qq_serif2.png", qq_plot, width = 16, height = 8, units = "in", dpi = 300)
...
$Z
 [1]  55.892652  55.921303  -3.014702  74.453482  14.508918  18.532180
 [7]  62.522285   3.227042   6.600983 -11.931197

$P
 [1] 0.000000e+00 0.000000e+00 1.286160e-03 0.000000e+00 5.319247e-48
 [6] 5.679563e-77 0.000000e+00 6.253859e-04 2.042204e-11 4.069742e-33

$P.adjusted
 [1] 0.000000e+00 0.000000e+00 1.286160e-02 0.000000e+00 5.319247e-47
 [6] 5.679563e-76 0.000000e+00 6.253859e-03 2.042204e-10 4.069742e-32

$comparisons
 [1] "built - control"     "built - mini-grid"   "control - mini-grid"
 [4] "built - ocean"       "control - ocean"     "mini-grid - ocean"  
 [7] "built - rural"       "control - rural"     "mini-grid - rural"  
[10] "ocean - rural"      
