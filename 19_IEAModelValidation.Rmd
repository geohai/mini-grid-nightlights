...
st_write(mgs, "data/oemap/mgs_iea_114_with_cell_id.geojson")
```


## Get Background Map to Visualize a Mini-Grid Site
```{r}
# read in the mini-grid sites
mgs <- st_read("data/oemap/mgs_iea_114_with_cell_id.geojson")

# get the center of the first mini-grid site
center <- c(
  lon = st_coordinates(mgs$geometry[1])[1, 1],
  lat = st_coordinates(mgs$geometry[1])[1, 2]
)
...
# # export map
# saveRDS(map, "data/oemap/mg_1_map.rds")
```
...
saveRDS(map, "data/oemap/mg_7_satmap.rds")

ggmap(map) + 
  geom_sf(data = sample_mg, fill = NA, color = "yellow", lwd = 1, inherit.aes = FALSE)
# export this figure
ggsave("figures/oemap/mg_7_satmap.png", width = 12, height = 12, units = "in", dpi = 300)

rm(sample_mg, center, map)
...
# export inter as RDS
saveRDS(inter, "data/oemap/inter_7.rds")
```

## Plot the Results
```{r}
# import data and map
map <- readRDS("data/oemap/mg_7_satmap.rds")
inter <- readRDS("data/oemap/inter_7.rds")
inter <- inter %>%
...
ggsave("figures/oemap/density_by_building_7.png", width = 12, height = 6, units = "in", dpi = 300)
...
ggplot(inter) +
  ...
  theme_bw() 
  # theme(text = element_text(size = 24, family = "serif"), legend.position = "bottom")
ggsave("figures/oemap/access_prob_by_distance_7.png", width = 12, height = 6, units = "in", dpi = 300)
...
ggsave("figures/oemap/access_prob_by_building_geom_sf_7.png", width = 12, height = 12, units = "in", dpi = 300)

# get bbox of inter
bbox <- st_bbox(inter)
names(bbox) <- c("left", "bottom", "right", "top")
# get a new stadia map centered around these coordinates
map <- get_stadiamap(
  bbox = bbox, 
  zoom = 17, 
  maptype = "stamen_toner_lite"
)
# plot buildings with stadia background
ggmap(map) +
  geom_sf(data = inter, aes(fill = prob_elec_access), inherit.aes = FALSE) +
  geom_sf(data = sample_mg, fill = NA, color="blue", inherit.aes = FALSE) +
  # geom_sf(data = sample_mg_pt, color = "blue", size = 5, inherit.aes = FALSE) +
  ...
  # add a scale bar
  # scalebar(x.min = left, x.max = right, y.min = bottom, y.max = top, dist = 1000, dist_unit = "m", st.dist = 0.1, st.dist_unit = "km")
ggsave("figures/oemap/access_prob_by_building_stadia_7.png", width = 12, height = 12, units = "in", dpi = 300)
