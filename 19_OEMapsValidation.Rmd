...
# put the distance and prob_elec_access columns from the results into their own data frame as pairs
results_by_building <- lapply(results, function(x) (
  data.frame(
    distance = x$distance, 
    prob_elec_access = x$prob_elec_access
  )
))
# put each pair into its own row of a data frame
results_by_building <- do.call(rbind, results_by_building)
# export 
saveRDS(results_by_building, "data/oemap/results_by_building_rnd2.rds")

# convert results to a data frame
results.df <- do.call(rbind, results)
# export the results as RDS
saveRDS(results, "data/oemap/results_rnd2.rds")
# read in the results
results.df <- readRDS("data/oemap/results_rnd2.rds")

# add results to mgs
mgs <- cbind(mgs, results.df)
...
# save mgs
mgs <- mgs %>% select(-distance, -prob_elec_access)
